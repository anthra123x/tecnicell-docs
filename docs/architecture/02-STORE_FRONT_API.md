...
| Method | Path | Purpose |
|--------|------|---------|
| `GET` | `/api/ecommerce/products` | List visible products with pagination, filtering, search |
| `GET` | `/api/ecommerce/products/[id]` | Single product detail (by ID or slug) |
| `POST` | `/api/orders` | Create a new order from the storefront |
| `GET` | `/api/orders/[id]` | Order tracking/lookup (by ID or externalReference) |
